...
def check_keydown_events(event, ai_settings, screen, ship, bullets, stats):
    if event.key == pygame.K_RIGHT: ship.moving_right = True
        ...
        flush_high_score(ai_settings, stats)
        pygame.quit()
        sys.exit()
...
def get_number_aliens_x(screen, ai_settings, alien_width):
    available_space_x = ai_settings.screen_width - 2*alien_width
#     available_space_x = screen.display.get_surface() - 2*alien_width
...
def update_aliens(ai_settings, stats, screen, sb, ship, aliens, bullets):
    ...
    aliens.update()
    if pygame.sprite.spritecollideany(ship, aliens): ship_hit(ai_settings, stats, screen, sb, ship, aliens, bullets)
...
def create_fleet(ai_settings, screen, ship, aliens):
    alien = Alien(ai_settings, screen)
    number_aliens_x = get_number_aliens_x(screen, ai_settings, alien.rect.width)
    number_rows = get_number_rows(ai_settings, ship.rect.height, alien.rect.height)
    for row_number in range(number_rows):
        for alien_number in range(number_aliens_x): create_alien(ai_settings, screen, aliens, alien_number, row_number)
...
def change_fleet_direction(ai_settings, aliens):
    for alien in aliens.sprites(): alien.rect.y += ai_settings.fleet_drop_speed
...
def flush_high_score(ai_settings, stats):
    with open(ai_settings.history_score_file) as f2:
        tmpscore = json.load(f2)
        tmpscore.append(stats.high_score)
        sleep(0.5)
    with open(ai_settings.history_score_file, 'w') as f3:
        json.dump(tmpscore, f3)   
